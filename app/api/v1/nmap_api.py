...
from app.api.v1.models import ScanSession, ResultCode, DiffResult
...
@app.get('/diff/{host}')
async def get_diff(host=Annotated[str, Path(title='hostname to scan')]) -> DiffResult:
    ...
    return ctrl.get_diff_result(host, scan_ids[-1], scan_ids[-2])
